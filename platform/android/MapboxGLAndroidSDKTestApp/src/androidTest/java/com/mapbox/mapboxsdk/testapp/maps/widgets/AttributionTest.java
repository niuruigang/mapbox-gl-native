...
public class AttributionTest {
  ...
  public void testImproveMapLink() {
    ...
    String locationData = "#/0.000000/0.000000/1";

    // test for trigger url intent
    Uri uri = Uri.parse(urlSpans[2].getURL() + locationData);
    Matcher<Intent> expectedIntent = allOf(hasAction(Intent.ACTION_VIEW), hasData(uri));
    intending(expectedIntent).respondWith(new Instrumentation.ActivityResult(0, null));
